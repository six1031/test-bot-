...
class Database:
    ...
    async def add_ticket_panel(self, guild_id, channel_id, message_id, panel_type):
        async with self.pool.acquire() as conn:
            await conn.execute("""
    ...
    async def get_ticket_panels(self):
        async with self.pool.acquire() as conn:
            return await conn.fetch("SELECT * FROM ticket_panels")
    ...
    async def create_ticket(self, channel_id, owner_id, ticket_type):
        async with self.pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO tickets
    ...
    async def get_open_ticket(self, owner_id, ticket_type):
        async with self.pool.acquire() as conn:
